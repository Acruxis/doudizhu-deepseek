...
- 🤖 四档 AI 难度
  ...
  - **中等**：偏好大组合清牌、保留大牌
  - **困难**：启发式评分 + 队友默契
  - **AI（大模型）**：通过 OpenAI 兼容接口决定出牌；模型策略非法时由困难策略接管并记录，接口无法联通时暂停报错
...
- **默认难度**：困难（本地策略）；只有主动选择“AI（大模型）”才会调用网络接口。
...
开发环境的配置会持久化到项目根目录 `config.ini`；PyInstaller 单文件版本则保存到 exe 同目录。
`config.ini` 已加入 Git ignore，可参照 `config.example.ini`，不要提交真实 API Key。
“AI（大模型）”档会调用该 OpenAI 兼容接口出牌。模型返回的牌会依次经过手牌归属、牌型和
压牌规则校验。**模型响应格式错误或出牌非法时，本回合由本地困难策略接管，并在状态区累计
失败次数、在历史中记录原因；断网、无 Key、超时或接口异常时仍会暂停当前回合并明确报错。**

每次模型调用都是独立的无状态请求：固定 system prompt 规定角色、斗地主规则和输出格式，
user prompt 则携带当前手牌、公开底牌、各家余牌、记牌统计、本局完整出牌历史和当前桌面牌。
已接受的模型出牌会通过游戏历史进入下一次快照，但模型的解释或错误回复不会被重复带入。
